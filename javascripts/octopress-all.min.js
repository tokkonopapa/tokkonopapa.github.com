...
// v0.1 (c) Kyle Simpson
// MIT License
(function(e){var k=e.setTimeout,g=e.document,l=0;e.jXHR=function(){function h(){try{a.parentNode.removeChild(a)}catch(b){}}function i(){d=!1;c="";h();a=null;f(0)}function j(a){try{b.onerror.call(b,a,c)}catch(n){throw Error(a);}}function m(){if(!(this.readyState&&"complete"!==this.readyState&&"loaded"!==this.readyState||d))this.onload=this.onreadystatechange=null,d=!0,4!==b.readyState&&j("Script failed to load ["+c+"]."),h()}function f(a,c){c=c||[];b.readyState=a;"function"===typeof b.onreadystatechange&&
b.onreadystatechange.apply(b,c)}var c,d,a,b=null,b={onerror:null,onreadystatechange:null,readyState:0,open:function(a,d){i();internal_callback="cb"+l++;(function(a){e.jXHR[a]=function(){try{f.call(b,4,arguments)}catch(d){b.readyState=-1,j("Script failed to run ["+c+"].")}e.jXHR[a]=null}})(internal_callback);c=d.replace(/=\?/,"=jXHR."+internal_callback);f(1)},send:function(){k(function(){a=g.createElement("script");a.setAttribute("type","text/javascript");a.onload=a.onreadystatechange=function(){m.call(a)};
a.setAttribute("src",c);g.getElementsByTagName("head")[0].appendChild(a)},0);f(2)},setRequestHeader:function(){},getResponseHeader:function(){return""},getAllResponseHeaders:function(){return[]}};i();return b}})(window);
...
function addSidebarToggler(){$("body").hasClass("sidebar-footer")||($("#content").append('<span class="toggle-sidebar"></span>'),$(".toggle-sidebar").bind("click",function(a){a.preventDefault();$("body").hasClass("collapse-sidebar")?$("body").removeClass("collapse-sidebar"):$("body").addClass("collapse-sidebar")}));var a=$("aside.sidebar > section");1<a.length&&a.each(function(b,c){3<=a.length&&0===c%3&&$(b).addClass("first");$(b).addClass((c+1)%2?"odd":"even")});3<=a.length&&$("aside.sidebar").addClass("thirds")}
...
function addCodeLineNumbers(){"Microsoft Internet Explorer"!==navigator.appName&&$("div.gist-highlight").each(function(a){for(var b='<pre class="line-numbers">',c=$(".line",a).length,d=1;d<=c;d++)b+='<span class="line-number">'+d+"</span>\n";b='<table><tbody><tr><td class="gutter">'+b+'</pre></td><td class="code"><pre>'+$("pre",a).html()+"</pre></td></tr></tbody></table>";$(a).html(b)})}
function flashVideoFallback(){$("video").each(function(a){a=$(a);if(!Modernizr.video.h264&&swfobject.getFlashPlayerVersion()||-1!==window.location.hash.indexOf("flash-test"))a.children("source[src$=mp4]").first().map(function(b){b=$(b).attr("src");var c="video_"+Math.round(1+1E5*Math.random()),d=a.attr("width"),e=parseInt(a.attr("height"),10)+30;a.after('<div class="flash-video"><div><div id='+c+">");swfobject.embedSWF("/assets/jwplayer/player.swf",c,d,e+30,"9.0.0",{file:b,image:a.attr("poster"),
skin:"/assets/jwplayer/glow/glow.xml"},{movie:b,wmode:"opaque",allowfullscreen:"true"})}),a.remove()})}function wrapFlashVideos(){$("object").each(function(a){a=$(a);if($("param[name=movie]",a).length){var b=a.before('<div class="flash-video"><div>').previous();$(b).children().append(a)}});$("iframe[src*=vimeo],iframe[src*=youtube]").each(function(a){a=$(a);var b=a.before('<div class="flash-video"><div>').previous();$(b).children().append(a)})}
function renderDeliciousLinks(a){for(var b="<ul>",c=0,d=a.length;c<d;c++)b+='<li><a href="'+a[c].u+'" title="Tags: '+(""==a[c].t?"":a[c].t.join(", "))+'">'+a[c].d+"</a></li>";$("#delicious").html(b+"</ul>")}$.domReady(function(){testFeatures();wrapFlashVideos();flashVideoFallback();addCodeLineNumbers();getNav();addSidebarToggler()});

// iOS scaling bug fix
// Rewritten version
// By @mathias, @cheeaun and @jdalton
// Source url: https://gist.github.com/901295
(function(a){function c(){b.content="width=device-width,minimum-scale="+d[0]+",maximum-scale="+d[1];a.removeEventListener(e,c,!0)}var e="gesturestart",d=[1,1],b="querySelectorAll"in a?a.querySelectorAll("meta[name=viewport]"):[];if((b=b[b.length-1])&&"addEventListener"in a)c(),d=[0.25,1.6],a.addEventListener(e,c,!0)})(document);
...
// github.js
//
var github=function(){return{showRepos:function(d){$.ajax({url:"https://api.github.com/users/"+d.user+"/repos?callback=?",type:"jsonp",error:function(){$(d.target+" li.loading").addClass("error").text("Error loading feed")},success:function(a){var b=[];if(a&&a.data){for(var c=0;c<a.data.length;c++)(!d.skip_forks||!a.data[c].fork)&&b.push(a.data[c]);b.sort(function(a,c){var b=(new Date(a.pushed_at)).valueOf(),d=(new Date(c.pushed_at)).valueOf();return b===d?0:b>d?-1:1});d.count&&b.splice(d.count);
a=0;var c="",e=$(d.target)[0];for(a=0;a<b.length;a++)c+='<li><a href="'+b[a].html_url+'">'+b[a].name+"</a><p>"+$("<div/>").text(b[a].description||"").html()+"</p></li>";e.innerHTML=c}}})}}}();

// JSON-P Twitter fetcher for Octopress
// (c) Brandon Mathis // MIT License

/* Sky Slavin, Ludopoli. MIT license.  * based on JavaScript Pretty Date * Copyright (c) 2008 John Resig (jquery.com) * Licensed under the MIT license.  */
function prettyDate(a){if("Microsoft Internet Explorer"===navigator.appName)return"<span>&infin;</span>";a=((new Date).getTime()+6E4-(new Date(a)).getTime())/1E3;var b=Math.floor(a/86400);return isNaN(b)||0>b?"<span>&infin;</span>":0===b&&(60>a&&" now"||120>a&&"1m"||3600>a&&Math.floor(a/60)+"m"||7200>a&&"1h"||86400>a&&Math.floor(a/3600)+"h")||1===b&&"1d"||7>b&&b+"d"||7===b&&"1w"||7<b&&Math.ceil(b/7)+"w"}
function linkifyTweet(a,b){a=a.replace(/(https?:\/\/)([\w\-:;?&=+.%#\/]+)/gi,'<a href="$1$2">$2</a>').replace(/(^|\W)@(\w+)/g,'$1<a href="https://twitter.com/$2">@$2</a>').replace(/(^|\W)#(\w+)/g,'$1<a href="https://search.twitter.com/search?q=%23$2">#$2</a>');for(var c in b)if(null!=b[c].expanded_url){var d=RegExp(b[c].url,"g");a=a.replace(d,b[c].expanded_url);d=RegExp(">"+b[c].url.replace(/https?:\/\//,""),"g");a=a.replace(d,">"+b[c].display_url)}return a}
function showTwitterFeed(a,b){var c=document.getElementById("tweets"),d="",e;for(e in a)d+='<li><p><a href="https://twitter.com/'+b+"/status/"+a[e].id_str+'">'+prettyDate(a[e].created_at)+"</a>"+linkifyTweet(a[e].text.replace(/\n/g,"<br>"),a[e].entities.urls)+"</p></li>";c.innerHTML=d}
function getTwitterFeed(a,b,c){b=parseInt(b,10);$.ajax({url:"https://api.twitter.com/1/statuses/user_timeline/"+a+".json?trim_user=true&count="+(b+20)+"&include_entities=1&exclude_replies="+(c?"0":"1")+"&callback=?",type:"jsonp",error:function(){$("#tweets li.loading").addClass("error").text("Twitter's busted")},success:function(c){showTwitterFeed(c.slice(0,b),a)}})};